...
class ResponsesApiMessageWrapper(ChatMessageWrapper):
    ...
    def content(self) -> str | None:
        ...
        if content is not None:
            ...
            dict_content = self.input_.get("content")
            if dict_content is not None:
                return cast(Optional[str], dict_content)
            return cast(Optional[str], self.input_.get("text"))
        text_value = getattr(self.input_, "text", None)
        if text_value is not None:
            return cast(Optional[str], text_value)
        return None
